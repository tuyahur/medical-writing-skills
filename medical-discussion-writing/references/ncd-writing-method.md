# 非传染性疾病流行病学Discussion：从结果到论证

适用：观察性队列、横断面、证据综合和MR。先识别研究设计及估计对象，再迁移结构；同一篇论文不同分析模块可以有不同推理上限。以下句架为自拟教学模板，不是论文原文，也不是语料频次统计。

## 1. 先写中心命题，再决定段落

中心命题：在[人群]中，[明确暴露及参照]与[明确结局]呈[方向/形状]关联；相对已有研究，新增[一个主要贡献]；能够支持[推论上限]。

从引言末段提出的问题，核对主结果和结论。Discussion的每段至少执行一种任务：回答问题、解释差异、支持可信度、限定推论或说明增量。仅有“该疾病很重要”“某通路很复杂”的段落若不能接回具体结果，应删减。

不要把八段模板当作通用规则。根据结果选择以下任务：

|段落任务|内部论证|服务全文的方式|主要检查|
|---|---|---|---|
...
### 内部检验使文献差异解释更有力度（Fang）

弱：Our findings differed because of methodological differences.

自拟较强表达：The association was weaker than in previous studies. Estimates attenuated after additional adjustment for cumulative smoking exposure, suggesting that differences in smoking control may partly account for this discrepancy.

关键是内部变化与解释路径对应；不是因为增加一个may就严密。调整也可能带来过度调整或碰撞偏倚，要先判断协变量角色。余下差异仍可由人群或暴露构成解释。

### 给阴性结果一个正确的估计对象（Li、Sabia）
...
文献矛盾→潜伏疾病可能影响暴露→本研究评估年龄/随访长度→对应分析→仍未排除的反向路径。这个顺序让方法成为论证的一部分，而不是Discussion重新写Methods。

### 让优势与问题一一对应（Ekelund、Sabia）

自拟句架：Repeated assessments allowed us to characterize changes in exposure over time, addressing a limitation of studies based on a single baseline measurement.

不要仅列large sample, prospective design, objective measurement。说明设备测量减少什么自报依赖、协调处理增强什么可比性、重复测量改善什么暴露定义。全国调查具有抽样优势，但分析样本缺失/排除和权重仍决定实际外推；样本来源代表性不自动传递给最终估计。

### 三角互证看假设差异，而非研究数量（Wootton）
...
R本研究结果；E外部证据；I解释；C比较；B边界；A应用/研究建议；T路标；S综合；M方法/优势。一个句子可多标签。

“Our sensitivity analysis showed…”是内部结果R/M，不能因其提供支持就标成外部证据E。“These results may reflect…”是I，本研究结果只是先行项；may不把推断变成事实。“Whether X affects Y remains uncertain”包含affects，但整个命题是未知，不能按动词字面判定因果主张。

人工注释至少记录：定位ID、句子的具体任务、证据来自本研究还是作者引用/解释、所支持的主张、尚不能支持的强命题。原作者引用的外部事实若未逐项打开，不标为本次独立核实。

## 4. 词汇先在词组和命题里判断

|类别|典型表达|真实作用|易误读|
|---|---|---|---|
|内部证据归属|we observed; our results|锚定结果来源|our findings后的每个主张不一定都是测得的|
|统计关系|associated with|保留关联语义|adjusted/independent不能消除所有混杂|
|比较|compared with; whereas|固定参照/并列不同结果|两个HR排序不等于直接比较显著|
|解释强度|may; might; could; suggest|限制解释的确定度|不能拯救没有证据路径的机制|
|转折/让步|however; although; nevertheless|切换证据方向或限制范围|下一句必须确实形成这种关系|
|推导|therefore; thus|由前提推出结论|观察结果不能凭连接词推出干预效果|
|否定/知识缺口|not; no; unclear; limited|分别否定结果、推论或证据充分性|no evidence不是evidence of no effect|
|范围限定|in midlife; for mortality; among…|锁定生命阶段、结局和人群|发病/死亡与一般认知/痴呆不能替换|
|内容名词|exposure; dementia; instrument|建立构念与估计对象|不应强分成观点或转折|
|语法关系|a/the; of; be; that/to/as|建立限定、补语、时态或关系|不是所有that是回指，to可能是不定式|

全量逐词表可做位置索引与规则初标；功能词、多义词、否定作用域、情态词指向和词组关系必须在语境中复核。自动词典不能命名为全量人工语义分析。抽取词组时保留整句ID，统计单词时不能误把PM2.5、作者姓名或数字处理为有意义的写作规范。
...
这六篇是目的性教学样本，英语表达会受主题、设计、期刊和讨论长度影响。报告总次数、论文覆盖数、每篇频次、每千词频次和检索时间；长论文可能主导合计。高频表示在该语料出现多，不证明它更清楚、更有说服力。要检验写作建议，需在未入选、不同期刊/设计的全文上复核，并审查实际改稿是否使主张与证据更一致。
